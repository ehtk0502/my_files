...
ArpCache::periodicCheckArpRequestsAndCacheEntries(){
    ...
    for (auto it = m_arpRequests.begin(); it != m_arpRequests.end();){
        if((*it)->nTimesSent < 5){
            (*it)->nTimesSent = (*it)->nTimesSent + 1;
            (*it)->timeSent = steady_clock::now();
            
            uint32_t nextHop = (*it)->ip;
            
            std::list<PendingPacket> m_packetList = (*it)->packets;
            std::list<PendingPacket>::iterator my_it = m_packetList.begin();
            const Interface* iface = m_router.findIfaceByName(my_it->iface);
            ...
            arpFill->arp_tip = nextHop;
            
            m_router.sendPacket(FillPacket,my_it->iface);
    ...
    /*
    for (auto& arpEntry : m_cacheEntries) {
        if (arpEntry->isValid == false){
            m_cacheEntries.erase(arpEntry);
        }
    }
    */
    //m_router;
