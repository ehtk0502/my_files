...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        def softmax_loss(x, y):
            ...
            #shape (N, C)
            expScore = np.exp(x)
            denom = np.sum(expScore, axis = 1)
            softMaxF = np.zeros(expScore.shape)
            
            for i in range(denom.size):
                softMaxF[i] = expScore[i] / denom[i]
            
            logSoft = np.log(softMaxF)
            loss = np.sum(softMaxF) / N
            
            for i in range(numUnique.size):
                for j in range(y_target.size):
                    if(y_target[j] == numUnique[i]):
                        decode[j,i] = 1
        ...
        def MSE_loss(y_pred, y_target):
            ...
            numUnique = np.unique(y_target)
            decode = np.zeros((y_target.size, numUnique.size))

            for i in range(numUnique.size):
                for j in range(y_target.size):
                    if(y_target[j] == numUnique[i]):
                        decode[j,i] = 1
            mse = np.square(y_pred -decode)
            
            loss = np.sum(mse) / (2 * N)
            dy_pred = (y_pred -decode)/N
            # ================================================================ #
            # END YOUR CODE HERE
            # ================================================================ #
            return loss, dy_pred
        ...
        w1quare = np.square(W1)
        w2quare = np.square(W2)
        reg_loss = (reg / 2) * (np.sum(w1quare) + np.sum(w2quare))
