...
def run(infile, min_support, min_conf):
    ...
    #print("this is set map")
    #print(set_count_map)
    #print("thisis freq map")
    #print(freq_map)
...
def gen_one_item_cand_set(input_fileator):
    ...
    for record in input_fileator:
        ...
        #one_cand_set.add(frozenset(transaction))
...
def get_items_with_min_support(item_set, all_transactions, min_support,
    ...
    # ============================ YOUR CODE HERE ============================ #
    for items in local_set:
        if(local_set[items] >= min_support):
            rtn.add(items)
        else:
            set_count_map.pop(items)
    # ============================ END YOUR CODE ============================ #
    ...
    for key, value in freq_map.items()[1:]:
        for item in value:
            ...
            for element in _subsets:
                ...
                if len(remain) > 0:
                    confidence = float(set_count_map[item]) / float(set_count_map[element])
...
def join_set(s, l):
    ...
    unionSet = set()
    for items in s:
        for items1 in tempSet1:
            union = items.union(items1)
            if(len(union) == l):
               unionSet.add(union)
    return unionSet# =================== YOUR CODE HERE =================== #
