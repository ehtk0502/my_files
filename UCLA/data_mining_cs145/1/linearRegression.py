...
def getBeta(train_x, train_y):   
    ...
    mult = np.matmul(trans, train_x)
    mInverse = np.linalg.inv(mult)
    multXY= np.matmul(trans,train_y)
    result = np.matmul(mInverse, multXY)
    beta = result
    return beta
...
def getBetaBatchGradient(train_x, train_y, lr, num_iter):
	...
	########## Please Fill Missing Lines Here ##########
			Yi = train_y[i]
			rowX = train_x[i]
			xTran = np.transpose(rowX)
			xTMultB = np.matmul(xTran, beta)
			contentParen = xTMultB - Yi
			deriv = deriv + rowX.dot(contentParen)
		deriv = deriv / n
...
def getBetaStochasticGradient(train_x, train_y, lr):
    ...
    for iter in range(epoch):
        ...
        for i in range(n):
            ...
            Yi = train_y[idx]
            rowX = train_x[idx]
            Xtransp = np.transpose(rowX)
            xTMultB = np.matmul(Xtransp, beta)
            contentParen = Yi - xTMultB
            product = rowX.dot(contentParen)
            product = product.dot(lr)
            beta = beta + product	
    return beta
