...
def compute_avglogL(X,y,beta):
    ...
    for i in range(0 , n):
        xRow = X[i]
        xTran = np.transpose(xRow)
        xTMultB = np.matmul(beta, xTran)
        exp = np.exp(xTMultB)
        logVal = logVal + np.log(1 + exp)
	
    firstVar = -1 * np.matmul(y, X)
    yxMultB = np.matmul(firstVar, beta)
    avglogL = (yxMultB + logVal) / n
    return avglogL
...
def getBeta_BatchGradient(train_x, train_y, lr, num_iter, verbose):
    ...
    #update beta interactively
    for iter in range(0, num_iter):
        ########## Please Fill Missing Lines Here ##########
        xMultB = np.matmul(train_x, beta)
        pXB = train_y - sigmoid(xMultB)
        xtran = np.transpose(train_x)
        deriv = np.matmul(xtran, pXB)
		###########regularization###################
        #regular= 2*lr*beta
        #deriv = deriv + regular
		####################################
...
def getBeta_Newton(train_x, train_y, num_iter, verbose):
    ...
    ########## Please Fill Missing Lines Here ##########
    for iter in range(0, num_iter):
        xTrans = np.transpose(train_x)
        x2 = np.matmul(xTrans, train_x)
        xMultB = np.matmul(train_x, beta)
        sigXB = sigmoid(xMultB)
        product = np.matmul(sigXB, (1-sigXB))
        final = -(x2 * product)
        inverse = np.linalg.inv(final)
		
        pXB = train_y - sigmoid(xMultB)
        firstDeriv = np.matmul(xTrans, pXB)
        
        beta = beta - np.matmul(inverse, firstDeriv)
