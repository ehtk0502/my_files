...
bool findBiggerMask(RoutingTableEntry& a, RoutingTableEntry& b){
...
{
    ///
    std::list<RoutingTableEntry> m_routingTable(m_entries);
    m_routingTable.sort(findBiggerMask);
    
    auto m_ptr = std::find_if(m_routingTable.begin(), m_routingTable.end(), [ip] (const RoutingTableEntry& m_ptr) {
      uint32_t result = m_ptr.dest & m_ptr.mask;
      uint32_t result1 = ip & m_ptr.mask;
      return result == result1;
    });

    if (m_ptr == m_entries.end()) {
